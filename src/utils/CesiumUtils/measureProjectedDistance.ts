// measureProjectedDistance.ts
// 投影距离测量工具
import * as Cesium from 'cesium';

export default class MeasureProjectedDistance {
  ...
  private updateTooltip(position: Cesium.Cartesian3) {
    ...
    if (screenPos) {
      ...
      this.tooltip.innerText = `投影距离: ${this.calculateProjectedDistance([...this.positions, position]).toFixed(2)} 米`;
    }
  }
  ...
  /**
   * 计算投影距离（所有点投影到地球椭球面后的折线长度）
   */
  public calculateProjectedDistance(positions: Cesium.Cartesian3[]): number {
  ...
  public finish() {
    ...
    // 显示最终距离
    if (this.tooltip) {
      const total = this.calculateProjectedDistance(this.positions);
      this.tooltip.innerText = `投影距离测量完成\n总长度: ${total.toFixed(2)} 米`;
